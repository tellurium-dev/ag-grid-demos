...
> = ({ api, node }) => {
  ...
  const onDownloadClick = useCallback(() => {
    ...
    console.log("Download for:", node.data);
  }, [node]);

  const { data } = node;

  return (
    <div className={styles.buttonCell} style={{ display: "flex", alignItems: "center", gap: "4px" }}>
      ...
      <button
        ...
        style={{
          ...
          alignItems: "center",
          justifyContent: "center",
          opacity: data && (data.status === "draft" || data.status === "active") ? 1 : 0,
          cursor: data && (data.status === "draft" || data.status === "active") ? "pointer" : "not-allowed",
        }}
        onClick={data && (data.status === "draft" || data.status === "active") ? onViewMoreClick : undefined}
        title={data && (data.status === "draft" || data.status === "active") ? "Edit" : "Edit (not available)"}
        disabled={!data || !(data.status === "draft" || data.status === "active")}
      >
      ...
      <button
        ...
        style={{
          ...
          alignItems: "center",
          justifyContent: "center",
          opacity: data && (data.status === "completed" || data.status === "active") ? 1 : 0,
          cursor: data && (data.status === "completed" || data.status === "active") ? "pointer" : "not-allowed",
        }}
        onClick={data && (data.status === "completed" || data.status === "active") ? onDownloadClick : undefined}
        title={data && (data.status === "completed" || data.status === "active") ? "PDF" : "PDF (not available)"}
        disabled={!data || !(data.status === "completed" || data.status === "active")}
      >
        {/* PDF icon */}
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" style={{ display: "block" }}>
          <path
            d="M3 2C2.44772 2 2 2.44772 2 3V13C2 13.5523 2.44772 14 3 14H13C13.5523 14 14 13.5523 14 13V3C14 2.44772 13.5523 2 13 2H3Z"
            stroke="#1976d2"
            strokeWidth="1.5"
            fill="none"
          />
          <path
            d="M5 5H11M5 7H11M5 9H9M5 11H11"
            stroke="#1976d2"
            strokeWidth="1.5"
            strokeLinecap="round"
          />
      ...
      <button
        ...
        style={{
          ...
          alignItems: "center",
          justifyContent: "center",
          opacity: data && (data.status === "draft" || data.status === "paused") ? 1 : 0,
          cursor: data && (data.status === "draft" || data.status === "paused") ? "pointer" : "not-allowed",
        }}
        onClick={data && (data.status === "draft" || data.status === "paused") ? onRemoveClick : undefined}
        title={data && (data.status === "draft" || data.status === "paused") ? "Delete" : "Delete (not available)"}
        disabled={!data || !(data.status === "draft" || data.status === "paused")}
      >
      ...
      <button
        ...
        style={{
          ...
          justifyContent: "center",
          color: "#388e3c",
          opacity: data && data.status === "paused" ? 1 : 0,
          cursor: data && data.status === "paused" ? "pointer" : "not-allowed",
        }}
        onClick={data && data.status === "paused" ? () => {
          // Example approve logic, replace with your own
          api.applyTransaction({ update: [{ ...data, status: "active" }] });
        } : undefined}
        title={data && data.status === "paused" ? "Approve" : "Approve (not available)"}
        disabled={!data || data.status !== "paused"}
      >
